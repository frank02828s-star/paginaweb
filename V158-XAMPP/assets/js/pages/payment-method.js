...
function humanPayment(methodId) {
  ...
  function getStats() {
    ...
    let items = [];
    try {
      const raw = localStorage.getItem('pcbuilder_cart');
...
const list = document.getElementById('payment-list');
  ...
  function computePayableTotal() {
    ...
    const coupon = (localStorage.getItem('pcbuilder_coupon') || '').trim();
...
try {
  ...
  window.location.href = '/checkout/order-completion.html?token=' + encodeURIComponent(t);
} catch (_) {
  window.location.href = '/checkout/order-completion.html';
}
...
// Re-render si cambian datos en otra pestaña
    window.addEventListener('storage', (e) => {
      if (e.key === (window.STORAGE_KEYS ? STORAGE_KEYS.CART : 'pcbuilder_cart') || e.key === 'pcbuilder_shipping_method') {
        renderPaymentList();
        renderSummary();
  window.__pcbuilderRenderSummary = renderSummary;
      }
