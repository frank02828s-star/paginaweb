...
(function __guardCompletion(){
    ...
    const cartRaw = __lsGet((window.STORAGE_KEYS && STORAGE_KEYS.CART) ? STORAGE_KEYS.CART : 'pcbuilder_cart');
...
async function autoProvisionCheckoutAccount(){
  ...
  function redirectToWhatsapp(msg){
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        localStorage.removeItem('pcbuilder_cart');
        localStorage.removeItem('pcbuilder_cart_items');
        ...
        sessionStorage.removeItem('pcbuilder_cart');
        sessionStorage.removeItem('pcbuilder_cart_items');
  ...
  function __validateCompletionState(ctx){
    ...
    if (!Array.isArray(ctx.items) || !ctx.items.length) {
      return __goBackForFix('/cart.html', 'Tu carrito está vacío. Agrega productos antes de continuar.');
    }
  ...
  async function render(){
    ...
    const items = safeGetJson('pcbuilder_cart', []);
